...
export const getDataPlans = async (network: string) => {
  ...
  console.log("VTPASS PLANS RAW:", JSON.stringify(response.data, null, 2));
  ...
  const plans = (Array.isArray(variations) ? variations : [])
    .map((item: any) => ({
      name: item.name || item.variation_name || "Data Plan",
      variation_code: String(item.variation_code || item.code || ""),
      amount: Number(item.variation_amount || item.amount || 0),
    }))
    .filter((p: any) => p.variation_code && p.amount > 0);

  console.log("VTPASS PLANS COUNT:", plans.length);
  return plans;
};
...
export const buyData = async ({
  ...
  return {
    success,
    message: data?.response_description || data?.message || "Data purchase",
    data,
    provider: "vtpass",
  };
};
...
export const buyAirtime = async ({
  ...
  return {
    success,
    message: data?.response_description || data?.message || "Airtime purchase",
    data,
    provider: "vtpass",
  };
};
